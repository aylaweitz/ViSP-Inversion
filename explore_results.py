...
class DeSIRe_inverted_map:
    ...
    def read_map(self):
        ...
        inversion_file = os.path.join(self.base_dir, 'inv_res_prof.fits')
        observed_file  = os.path.join(self.base_dir, 'observed_prof.fits')
        models_file    = os.path.join(self.base_dir, 'inv_res_mod.fits')
    ...
    def draw_profiles(self, update=False):
        ...
        if update:
            if len(self.lines) > 1:
                ...
                Npx, = self.axs_prof.shape
                for i in range(Npx):
                     self.axs_prof[i].clear()
        ...
        for k in range(N_STOKES):
            for l in range(len(self.lines)):
                ...
                axs_prof_kl = self.axs_prof[k][l] if len(self.lines) > 1 else self.axs_prof[k]
                axs_prof_kl.plot(waves, \
                ...
                axs_prof_kl.plot(waves, \
                                         ...
                                         label='fit')
                if l == 0:
                    axs_prof_kl.set(ylabel=ylabels[k])

                if k == 0:
                    axs_prof_kl.set(ylim=[0.0, 1.2])
                    title_text = 'Line:  %6.6g nm' % \
                                  (self.lines[l].lambda0)
                    axs_prof_kl.title.set_text(title_text)

                if k == 0  and  l == 0:
                    axs_prof_kl.legend(loc='lower left')
                    axs_prof_kl.annotate('x = %d, y = %d' % \
                                                 (ix, iy), \
                                                 (0.65, 0.05), \
        ...
        if update:
            if self.button == 3:
                ...
                pdf_file = self.fig_dir + 'FeI_CaII_' + \
                              location_str + '.pdf'
...
def main():
    ...
    base_dir = os.path.join(home_dir,\
                            'Source/Python/DKIST/parallel_master_v0.7/results/')

    map = DeSIRe_inverted_map(base_dir=base_dir)
    map.read_map()
